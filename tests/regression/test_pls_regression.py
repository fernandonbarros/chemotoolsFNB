...
class TestPLSRegressionCompatibility:
    ...
    def test_works_with_column_transformer(self):
        ...
        # Add synthetic metadata columns to simulate a real-world scenario
        df_x.insert(0, "sample_id", [f"S{i:04d}" for i in range(len(df_x))])
        df_x.insert(1, "region",    np.random.choice(["North", "South", "East", "West"], size=len(df_x)))
        ...
        expected_columns = ["sample_id", "region"] + [f"LV{i+1}" for i in range(N_COMPONENTS)]
